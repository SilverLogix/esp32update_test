...
BATCH_SIZE = 1  # Number of files to process in a single batch, can be increased for people with PSRAM.
...
def process_batch(batch_files, ignore, internal_tree, log):
    for item in batch_files:
        if item['type'] == 'tree':
            ...
            try:
                content = pull_file(item['path'], raw_url)  # Remove the extra argument
                with open(item['path'], 'w') as file:
                    file.write(content)
                    file.close()  # Close the file explicitly after writing
